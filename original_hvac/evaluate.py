...
def evaluate(use_trained: bool = True):
    ...
    print(f"  {label}")
    print(f"{'='*55}")
    print(f"  Tza trung binh:    {np.mean(log['Tza']):.2f} oC  (max={np.max(log['Tza']):.2f})")
    print(f"  RH trung binh:     {np.mean(log['phi'])*100:.1f}%  (max={np.max(log['phi'])*100:.1f}%)")
    print(f"  CO2 trung binh:    {np.mean(log['CO2']):.0f} ppm")
    print(f"  PM2.5 trung binh:  {np.mean(log['PM']):.2f} ug/m3")
    print(f"  Tong nang luong:   {sum(log['E']):.3f} kWh/day")
    print(f"  Reward trung binh: {np.mean(log['r']):.3f}/step")
    print(f"\n  --- Vi pham ---")
    ...
    print(f"\n  --- Action trung binh (daytime) ---")
    occ = [i for i in range(96) if 6<=i*0.25<20]
    print(f"  T_chws: {np.mean([log['act_Tchws'][i] for i in occ]):.2f} oC")
